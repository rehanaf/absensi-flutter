...
class AdminParentChildRequestsScreen extends StatefulWidget {
  ...
  @override
  State<AdminParentChildRequestsScreen> createState() => _AdminParentChildRequestsScreenState();
}

class _AdminParentChildRequestsScreenState extends State<AdminParentChildRequestsScreen> {
  ...
  Future<void> _approveRequest(int id) async {
    ...
    try {
      ...
      if (mounted) {
        AppToast.showSuccess(context, message: result['message'] ?? 'Permintaan hubungan disetujui!');
        _fetchRequests();
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _rejectRequest(int id) async {
    ...
    try {
      ...
      if (mounted) {
        AppToast.showSuccess(context, message: result['message'] ?? 'Permintaan hubungan ditolak!');
        _fetchRequests();
      }
    } catch (e) {
      if (mounted) {
              ...
              : _requests.isEmpty
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemCount: _requests.length + (_currentPage < _lastPage ? 1 : 0),
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 16),
                                  _buildPersonRow(context, 'WALI / ORANG TUA', parentName, 'HP: $parentPhone', Icons.family_restroom),
                                  const Divider(height: 24),
                                  _buildPersonRow(context, 'ANAK / SISWA', childName, 'NIS: $childNIS', Icons.school),
  ...
  Widget _buildPersonRow(BuildContext context, String header, String name, String subtitle, IconData icon) {
    return Row(
      children: [
