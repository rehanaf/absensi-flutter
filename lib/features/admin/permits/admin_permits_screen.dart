...
class _AdminPermitsScreenState extends State<AdminPermitsScreen> {
  ...
  Future<void> _fetchItems() async {
    ...
    try {
      final data = await _apiService.getPermits(
        page: _currentPage,
        search: _searchQuery,
      );
      if (!mounted) return;
  ...
  Future<void> _deleteItem(int id) async {
    ...
    try {
      await _apiService.deletePermit(id);
      if (mounted) {
  ...
  void _navigateToForm([Map<String, dynamic>? item]) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AdminPermitFormScreen(item: item),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Manajemen Izin & Cuti'),
        actions: [
          IconButton(
