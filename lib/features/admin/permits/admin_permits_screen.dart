...
import 'admin_permit_form_screen.dart';

class AdminPermitsScreen extends StatefulWidget {
  const AdminPermitsScreen({super.key});

  @override
  State<AdminPermitsScreen> createState() => _AdminPermitsScreenState();
}

class _AdminPermitsScreenState extends State<AdminPermitsScreen> {
  ...
  static const Color _iconColor = Color(0xFFFFCA28);
  ...
  Future<void> _fetchData({int page = 1}) async {
    ...
    try {
      final res = await _apiService.getPermits(page: page, search: _search);
  ...
  Future<void> _deleteItem(dynamic item) async {
    final title = item['user']?['name'] ?? item['title'] ?? 'Tanpa Nama';
    final confirmed = await _showDeleteDialog(title);
    if (!confirmed) return;
    try {
      await _apiService.deletePermit(item['id']);
      AppToast.showSuccess(context, message: 'Izin berhasil dihapus');
  ...
  Future<bool> _showDeleteDialog(String name) async {
    return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text('Yakin ingin menghapus izin "\$name"?'),
            actions: [
              TextButton(
            ...
            ListTile(
              ...
              onTap: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => AdminPermitFormScreen(item: item),
                  ),
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => const AdminPermitFormScreen()),
        ).then((_) => _fetchData(page: _currentPage)),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text('Tidak ada data izin', style: TextStyle(color: cs.outline)),
          ],
        ),
      );
    }
