...
class _AdminRolesScreenState extends State<AdminRolesScreen> {
  ...
  static const _iconColor = Color(0xFF78909C);
  ...
  Future<void> _fetchItems() async {
    ...
    try {
      final data = await _apiService.getRoles(
        page: _currentPage,
        search: _searchQuery,
      );
      if (!mounted) return;
  ...
  void _navigateToForm([Map<String, dynamic>? item]) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AdminRoleFormScreen(item: item)),
    );
                                ...
                                child: Material(
                                  ...
                                  child: Column(
                                    children: _items.asMap().entries.map((entry) {
                                      ...
                                      final guardName = item['guard_name']?.toString() ?? '';
